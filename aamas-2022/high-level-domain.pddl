...
(:functions (energy ?r - robot)
            ...
            (take_image_dur)
            (check_pressure_dur)
            (check_temperature_dur)
            (valve_inspection_dur)
...
:duration (= ?duration comm_data_dur)
:condition (and
...
:parameters (?r - robot ?s - robot_sensor ?wp - poi)
:duration (= ?duration check_temperature_dur)
:condition (and
           (over all (robot_can_act ?r  ?wp))
           (over all (at ?r ?wp))
...
:parameters (?r - robot ?s - robot_sensor  ?wp - poi)
:duration (= ?duration check_pressure_dur)
:condition (and
           (over all (robot_can_act ?r  ?wp))
           (over all (at ?r ?wp))
           (at start (at ?r ?wp))
...
(:durative-action valve_inspection
 :parameters (?r - robot   ?s - robot_sensor ?wp - poi)
 :duration ( = ?duration valve_inspection_dur)
 :condition (and
             (over all (robot_can_act ?r  ?wp))
             (over all (at ?r ?wp))
